...
const vocabularyKey = (songId: string, cueId: string, tokenId: string): string =>
  `${songId}:${cueId}:${tokenId}`
...
export function App() {
  ...
  const songIsLearned = learnedIds.has(song.id)
  const selectionIsLearning = selection
    ? learning.vocabulary.some((item) => vocabularyKey(item.songId, item.cueId, item.tokenId) ===
      vocabularyKey(song.id, selection.cue.id, selection.token.id))
    : false
  ...
  const toggleSelectedVocabulary = async () => {
    ...
    try {
      if (selectionIsLearning) {
        await removeVocabulary(song.id, cue.id, token.id)
        setLearning((current) => ({
          ...current,
          vocabulary: current.vocabulary.filter((item) => vocabularyKey(item.songId, item.cueId, item.tokenId) !==
            vocabularyKey(song.id, cue.id, token.id)),
        }))
        setToast(`Removed “${token.text}” from learning words`)
      } else {
        const item = await saveVocabulary(song.id, cue.id, token.id)
        setLearning((current) => ({
          ...current,
          vocabulary: [item, ...current.vocabulary.filter((saved) => vocabularyKey(saved.songId, saved.cueId, saved.tokenId) !==
            vocabularyKey(item.songId, item.cueId, item.tokenId))],
        }))
        setToast(`Added “${token.text}” to learning words`)
      }
    } catch (error) {
  ...
  const toggleSongLearned = async () => {
    if (!user) {
      askToSignIn('Sign in to track learned songs.')
      return
    }
    const status = songIsLearned ? 'learning' : 'learned'
    try {
      ...
      setToast(songIsLearned ? 'Moved back to learning' : `Marked “${song.title}” as learned`)
    } catch (error) {
      ...
      <main className="main-stage">
        <header className="topbar">
          ...
          <button className="breadcrumbs breadcrumb-button" onClick={() => setCatalogOpen(true)}>
            <span>Song library</span><span className="breadcrumb-slash">/</span><strong>Now learning</strong>
          </button>
          ...
          <div className="lesson-column" id="lyrics">
            <section className="song-hero" aria-labelledby="song-title">
              ...
              <div className="hero-copy">
                <div className="hero-kicker"><span /> Now learning</div>
                ...
                <div className="hero-learning-actions">
                  <button
                    className={songIsLearned ? 'learned-button is-learned' : 'learned-button'}
                    onClick={() => void toggleSongLearned()}
                    aria-pressed={songIsLearned}
                  ><CheckIcon /> {songIsLearned ? 'Learned' : 'Mark as learned'}</button>
                </div>
